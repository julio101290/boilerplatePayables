...
<?= $this->include('julio101290\boilerplatepayables\Views\modulesNotasCredito/modaSendMail') ?>


<?= $this->include('julio101290\boilerplatepayables\Views\modulesNotasCredito/listaFacturas') ?>
<?= $this->include('julio101290\boilerplatepayables\Views\modulesNotasCredito/xmlList') ?>
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="table-responsive">
                    ...
                    <table id="tableNotasCredito" class="table table-striped table-hover va-middle tableNotasCredito">
...
<script>
    ...
    var tableNotasCredito = $('#tableNotasCredito').DataTable({
        processing: true,
        serverSide: true,
        responsive: true,
        autoWidth: false,
        ...
        ajax: {
            url: '<?= base_url('admin/notascredito') ?>',
            method: 'get',
            dataType: "json"
        },
        columnDefs: [{
                ...
                "data": function (data) {
                    ...
                    return `<td class="text-right py-0 align-middle">
                         <div class="btn-group btn-group-sm">
                             <a href="<?= base_url('admin/editNotaCredito') ?>/${data.UUID}" class="btn btn-primary btn-edit"><i class="fas fa-pencil-alt"></i></a>
    ...
    $("#chkTodasLasVentas").on("change", function () {
        ...
        tableNotasCredito.ajax.url(`<?= base_url('admin/notasCredito') ?>/` + desdeFecha + '/' + hastaFecha + '/' + todas + '/' + idEmpresa + '/' + idSucursal + '/' + idCliente).load();
    ...
    $(".tableNotasCredito").on("click", '.btnListXML', function () {
    ...
    $(".tableNotasCredito").on("click", '.btnInvoiceList', function () {
        ...
        tableInvoice.ajax.url(`<?= base_url('admin/xmlenlace/getXMLEnlazadosNotaCredito') ?>/` + uuid).load();
    ...
    $(".tableNotasCredito").on("click", '.btnSendMail', function () {
    ...
    $(".tableNotasCredito").on("click", '.btnTimbrar', function () {

        var uuid = $(this).attr("uuid");

        timbrarNotaCredito(uuid);
    ...
    function timbrarNotaCredito(UUID) {
    ...
    $(".tablePagos").on("click", '.btnImprimirVenta', function () {
        ...
        window.open("<?= base_url('admin/notasCredito/report') ?>" + "/" + uuid, "_blank");
    ...
    /*=============================================
     ELIMINAR custumers
     =============================================*/
    $(".tableNotasCredito").on("click", ".btn-delete", function () {

        var idNotaCredito = $(this).attr("data-id");
        ...
        Swal.fire({
            ...
            confirmButtonText: '<?= lang('boilerplate.global.sweet.confirm_delete') ?>'
                ...
                .then((result) => {
                    if (result.value) {
                        $.ajax({
                            url: `<?= base_url('admin/notascredito') ?>/` + idNotaCredito,
                            method: 'delete',
                        }).done((data, textStatus, jqXHR) => {
                            Toast.fire({
                                icon: 'success',
