...
use julio101290\boilerplatepayables\Models\{
    PaymentsModel
};
...
class PaymentsController extends BaseController {
    ...
    public function index() {
        if ($this->request->isAJAX()) {
            $datos = $this->payments->select('id,idPayable,importPayment,importBack,datePayment,metodPayment,created_at,updated_at,deleted_at')->where('deleted_at', null);
    ...
    public function save() {
        ...
        $datosVenta = $this->payables->select("*")->where("UUID", $datos["UUID"])->asArray()->first();

        $datos["idPayable"] = $datosVenta["id"];
        $datos["idQuote"] = "0";
        ...
        try {
            ...
            try {
                ...
                $this->payables->update($datosVenta["id"],  $newBalance);
    ...
    public function ctrGetPayments($uuid) {
        ...
        $payable = $this->payables->select("*")->where("UUID", $uuid)->first();
    ...
    public function delete($id) {
        ...
        $dataPayable = $this->payables->find($infoPayments["idPayable"]);

        $totalPago = $infoPayments["importPayment"] - $infoPayments["importBack"];

        $dataPayableSave["balance"] = $dataPayable["balance"] + $totalPago;
         ...
         * Actualizamos saldo
         */
        $resultVenta = $this->payables->update($dataPayable["id"], $dataPayableSave);
