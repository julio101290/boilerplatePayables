...
use julio101290\boilerplatepayables\Models\NotasCreditoModel;
...
use julio101290\boilerplatepayables\Models\NotaCreditoDetailsModel;
...
class NotasCreditoController extends BaseController {
        ...
        if ($existsNotaCredito == 0) {
            ...
            if ($empresa["facturacionRD"] == "on") {
                ...
                if ($datos["date"] < $comprobante["desdeFecha"]) {
                    ...
                    $this->payables->notasCredito->transRollback();
    ...
    public function delete($id) {
        ...
        foreach ($listPagos as $key => $value) {
            ...
            $payable = $this->payables->select("*")->where("id", $value["idPayable"])->first();

            $nuevoSaldo["balance"] = $payable["balance"] + $value["importeAPagar"];

            if ($this->payables->update($payable["id"], $nuevoSaldo) === false) {
  ...
  <br><br>
          ...
          <div style="font-size:8.5pt;text-align:left;font-weight:ligth">UUID DOCUMENTO: $dataPayables[UUID]</div>
    ...
    public function obtenerFacturasPendientes() {
        ...
        foreach ($listaFacturas as $key => $value) {
            ...
            $facturas .= <<<EOF
                ...
                <input disabled type="hidden" id="idPayable" class="form-control idPayable"  name="idPayable" value="$value[id]" required="">    </div>
