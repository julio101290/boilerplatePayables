...
class PayablesModel extends Model {
    ...
    public function mdlGetPayables($empresas, $params = []) {
        ...
        $builder = $this->db->table('payables a')
                ->select("
            a.UUID,
            a.id,
            {$nameExpression},
        ...
        // Total sin filtros
        $total = $this->db->table('payables a')
                ->join('custumers b', 'a.idCustumer = b.id', 'left')
                ->join('empresas c', 'a.idEmpresa = c.id', 'left')
                ->whereIn('a.idEmpresa', $empresas)
                ->countAllResults();
     ...
     * Search by filters
     */
    public function mdlGetPayablesFilters(
            $empresas, $from, $to, $allPayables,
        ...
        $totalBuilder = $this->db->table('payables a')
                ->join('custumers b', 'a.idCustumer = b.id', 'left')
                ->join('empresas c', 'a.idEmpresa = c.id', 'left')
                ->where('a.date >=', $from . ' 00:00:00')
                ->where('a.date <=', $to . ' 23:59:59')
     ...
     */
    public function mdlGetPayableUUID($uuid, $empresas) {
        $dbDriver = $this->db->getPlatform();

        // Concatenación para nombre del cliente
        $nameExpression = $dbDriver === 'Postgre' ? "(b.firstname || ' ' || b.lastname) AS \"nameCustumer\"" : "CONCAT(b.firstname, ' ', b.lastname) AS nameCustumer";

        $result = $this->db->table('payables a')
                ->select("
            a.idCustumer,
            a.folio,
            a.quoteTo,
    ...
    public function mdlVentasPorProductosAgrupado(
        ...
        $builder = $this->db->table('payables a')
                ...
                ->join('payablesdetails b', 'a.id = b.idPayable')
    ...
    public function mdlIVARetenidoTotales($id) {
        ...
        $result = $this->db->table('payablesdetails')
